...
bump N everywhere** (currently `v=8`):

```
grep -rl 'v=8' --include='*.html' . | xargs perl -pi -e 's/(style\.css|script\.js|nav\.css|nav\.js)\?v=8/$1?v=8/g'
perl -pi -e 's/(nav\.css|nav\.js)\?v=8/$1?v=8/g' assets/script.js      # BSD grep's --include skips explicit files — do this separately
```
