...
/privacy.html, terms.html        legal pages (depth 0, `main.legal`); linked from the home/404 footers and the `.fg-legal` line nav.js adds
/robots.txt, sitemap.xml         generated by tools/seo/build-head.py — do not hand-edit
/favicon.ico, assets/favicon.svg, apple-touch-icon.png, og.png   brand images from the hero's layered stack (tools/brand/build-brand.py)
...
/tools/seo/build-head.py         writes each page's `<!-- HEAD:seo -->` block (description, canonical, OG, icons) + sitemap.xml + robots.txt
/tools/brand/build-brand.py      rebuilds favicon / touch icon / social preview image (Chrome + Pillow; rarely needed)
...
6. Head blocks + sitemap — `python3 tools/seo/build-head.py` after adding, renaming or retitling any page (`git add` new
   pages first). Descriptions come from the hero subtitle; canonicals always point at christopherm.xyz so the GitHub
   Pages mirror is not a duplicate. Titles must be unique site-wide (`Chapter — Guide`). `--check` for CI/audit.
...
bump N everywhere** (currently `v=14`; the same applies to `fonts.css`, `home.js` and `404.js`):

```
git ls-files -z '*.html' | xargs -0 perl -pi -e 's/(style\.css|script\.js|nav\.css|nav\.js|fonts\.css|home\.js|404\.js|favicon\.svg|favicon\.ico|apple-touch-icon\.png)\?v=14/$1?v=15/g'
perl -pi -e 's/(nav\.css|nav\.js|fonts\.css)\?v=14/$1?v=15/g' assets/script.js      # not an .html file — do this separately
