...
bump N everywhere** (currently `v=7`):

```
grep -rl 'v=7' --include='*.html' . | xargs perl -pi -e 's/(style\.css|script\.js|nav\.css|nav\.js)\?v=7/$1?v=8/g'
perl -pi -e 's/(nav\.css|nav\.js)\?v=7/$1?v=8/g' assets/script.js      # BSD grep's --include skips explicit files — do this separately
```
