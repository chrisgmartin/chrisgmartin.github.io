...
bump N everywhere** (currently `v=12`):

```
git ls-files "*.html" | xargs perl -pi -e 's/(style\.css|script\.js|nav\.css|nav\.js)\?v=12/$1?v=13/g'
perl -pi -e 's/(nav\.css|nav\.js)\?v=12/$1?v=13/g' assets/script.js      # not an .html file — do this separately
```
