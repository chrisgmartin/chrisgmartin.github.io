...
bump N everywhere** (currently `v=9`):

```
grep -rl 'v=9' --include='*.html' . | xargs perl -pi -e 's/(style\.css|script\.js|nav\.css|nav\.js)\?v=9/$1?v=9/g'
perl -pi -e 's/(nav\.css|nav\.js)\?v=9/$1?v=9/g' assets/script.js      # BSD grep's --include skips explicit files — do this separately
```
...
- Type: Fraunces (headings), Inter (body), JetBrains Mono (labels/code), Patrick Hand (plate handwriting) — loaded by
  `script.js` on every page (homepage loads its own link).
